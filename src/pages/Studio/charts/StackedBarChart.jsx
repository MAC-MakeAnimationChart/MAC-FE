 ...
 * - GroupedBarChart와 비슷하지만, 같은 stackId를 사용해 막대를 하나로 쌓음
 */
import {
  BarChart,
  Bar,
...
export default function StackedBarChart({ headers, rows, chartConfig }) {
  ...
  const hasStackedData = data.length > 0 && yKeys.length > 0;

  if (!hasStackedData) {
    return (
      <div className="jay-racing-empty">
        누적 막대 차트로 표시할 수 있는 숫자형 데이터가 부족합니다.
      </div>
    );
  }
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        {yKeys.map((key, index) => {
          const isLastBar = index === yKeys.length - 1;
